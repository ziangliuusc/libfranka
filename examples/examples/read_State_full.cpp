 ...
 * read_State.cpp
 *
 * A minimal script for getting the robot state when not controlling the robot
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout << "Robot IP: " << argv[1] << std::endl;
    ...
    while (true) {
      ...
      std::array<double, 42> jacobian = model.zeroJacobian(franka::Frame::kEndEffector, current_state);
      for (int i = 0; i < 7; i++) {
